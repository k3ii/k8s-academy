# KubeCon / CNCF deep technical talk corpus, per depth area

Resolves issue #4 (`k3ii/k8s-academy`). Research date: 2026-08-17.

## Scope and selection rules applied
...
- **Fewer, better.** 40 entries are marked **★** (core — earns its runtime unconditionally); of those, **Appendix A names the 24 to watch first, in dependency order**. That 24 is the curated set the ticket asked for. Everything unmarked is a labelled second pass, kept only because it closes a stated gap or is cheap (under 30 min) relative to what it teaches — not because it deserves equal billing.
...
## Verification method — read this before trusting a link
...
## Appendix A — the ★ CORE 24, in suggested viewing order
...
| Appendix C pointer (SIG Scalability) | 0 | 1 | 1 |
| **Total** | **40** | **33** | **73 verified links** |

Of the 40 ★ entries, **Appendix A's ordered 24** is the set to actually commit to the curriculum; the other 16 ★ entries are phase-gated (they only make sense once their area opens).

*(One talk, `Controllers at Chaos`, is cross-listed in areas 5 and 13 and is counted once, in area 5.)*

## Appendix C — known gaps, stated rather than papered over
