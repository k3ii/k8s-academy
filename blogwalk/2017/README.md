...
| 08 | [The project this post announces survives in the pinned documentation as a tab id, a PNG filename and an AppArmor profile; the runtime it was written to displace has six migration pages, a glossary entry and a note saying it does not implement CRI at all; and of this curriculum's two hand-edits to `config.toml` one is gone because two defaults agreed, and the other is the answer the kubelet now asks for instead of reading its own setting](08-containerd-container-runtime-options-kubernetes.md) | written |
